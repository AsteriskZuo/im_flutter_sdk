...
- (void)handleMethodCall:(FlutterMethodCall*)call
    ...
    if ([ExtSdkMethodKeySendMessage isEqualToString:call.method]) {
        [self sendMessage:call.arguments
              channelName:ExtSdkMethodKeySendMessage
                   result:result];
    } else if ([ExtSdkMethodKeyResendMessage isEqualToString:call.method]) {
        [self resendMessage:call.arguments
                channelName:ExtSdkMethodKeyResendMessage
                     result:result];
    } else if ([ExtSdkMethodKeyAckMessageRead isEqualToString:call.method]) {
        [self ackMessageRead:call.arguments
                 channelName:ExtSdkMethodKeyAckMessageRead
                      result:result];
    } else if ([ExtSdkMethodKeyAckGroupMessageRead isEqualToString:call.method]) {
        [self ackMessageRead:call.arguments
                 channelName:ExtSdkMethodKeyAckMessageRead
                      result:result];
    } else if ([ExtSdkMethodKeyAckConversationRead isEqualToString:call.method]) {
        [self ackConversationRead:call.arguments
                      channelName:ExtSdkMethodKeyAckConversationRead
                           result:result];
    }
    else if ([ExtSdkMethodKeyRecallMessage isEqualToString:call.method]) {
        [self recallMessage:call.arguments
                channelName:ExtSdkMethodKeyRecallMessage
                     result:result];
    } else if ([ExtSdkMethodKeyGetConversation isEqualToString:call.method]) {
        [self getConversation:call.arguments
                  channelName:ExtSdkMethodKeyGetConversation
                       result:result];
    } else if ([ExtSdkMethodKeyGetMessage isEqualToString:call.method]) {
        [self getMessageWithMessageId:call.arguments
                          channelName:ExtSdkMethodKeyGetMessage
                               result:result];
    }  else if ([ExtSdkMethodKeyMarkAllChatMsgAsRead isEqualToString:call.method]) {
        [self markAllMessagesAsRead:call.arguments
                        channelName:ExtSdkMethodKeyMarkAllChatMsgAsRead
                             result:result];
    } else if ([ExtSdkMethodKeyGetUnreadMessageCount isEqualToString:call.method]) {
        [self getUnreadMessageCount:call.arguments
                        channelName:ExtSdkMethodKeyGetUnreadMessageCount
                             result:result];
    } else if ([ExtSdkMethodKeyUpdateChatMessage isEqualToString:call.method]) {
        [self updateChatMessage:call.arguments
                    channelName:ExtSdkMethodKeyUpdateChatMessage
                         result:result];
    } else if ([ExtSdkMethodKeyDownloadAttachment isEqualToString:call.method]) {
        [self downloadAttachment:call.arguments
                     channelName:ExtSdkMethodKeyDownloadAttachment
                          result:result];
    } else if ([ExtSdkMethodKeyDownloadThumbnail isEqualToString:call.method]) {
        [self downloadThumbnail:call.arguments
                    channelName:ExtSdkMethodKeyDownloadThumbnail
                         result:result];
    } else if ([ExtSdkMethodKeyImportMessages isEqualToString:call.method]) {
        [self importMessages:call.arguments
                 channelName:ExtSdkMethodKeyImportMessages
                      result:result];
    } else if ([ExtSdkMethodKeyLoadAllConversations isEqualToString:call.method]) {
        [self loadAllConversations:call.arguments
                       channelName:ExtSdkMethodKeyLoadAllConversations
                            result:result];
    } else if ([ExtSdkMethodKeyGetConversationsFromServer isEqualToString:call.method]) {
        [self getConversationsFromServer:call.arguments
                             channelName:ExtSdkMethodKeyGetConversationsFromServer
                                  result:result];
    } else if ([ExtSdkMethodKeyDeleteConversation isEqualToString:call.method]) {
        [self deleteConversation:call.arguments
                     channelName:ExtSdkMethodKeyDeleteConversation
                          result:result];
    } else if ([ExtSdkMethodKeyFetchHistoryMessages isEqualToString:call.method]) {
        [self fetchHistoryMessages:call.arguments
                       channelName:ExtSdkMethodKeyFetchHistoryMessages
                            result:result];
    } else if ([ExtSdkMethodKeySearchChatMsgFromDB isEqualToString:call.method]) {
        [self searchChatMsgFromDB:call.arguments
                      channelName:ExtSdkMethodKeySearchChatMsgFromDB
                           result:result];
    } else if ([ExtSdkMethodKeyUpdateConversationsName isEqualToString:call.method]) {
        [self updateConversationsName:call.arguments
                          channelName:ExtSdkMethodKeyUpdateConversationsName
                               result:result];
    } else if ([ExtSdkMethodKeyAsyncFetchGroupAcks isEqualToString:call.method]) {
        [self fetchGroupReadAck:call.arguments
                    channelName:ExtSdkMethodKeyAsyncFetchGroupAcks
                         result:result];
    }
    else {
...
- (void)sendMessage:(NSDictionary *)param
    ...
    [EMClient.sharedClient.chatManager sendMessage:msg
        ...
        [weakSelf.messageChannel invokeMethod:ExtSdkMethodKeyOnMessageProgressUpdate
                                    arguments:@{
            @"progress":@(progress),
        ...
        if (error) {
            [weakSelf.messageChannel invokeMethod:ExtSdkMethodKeyOnMessageError
                                        arguments:@{
                @"error":[error toJson],
                @"localTime":@(msg.localTime),
            ...
            [weakSelf.messageChannel invokeMethod:ExtSdkMethodKeyOnMessageSuccess
                                        arguments:@{
                @"message":[message toJson],
                @"localTime":@(msg.localTime)
            }];
...
- (void)conversationListDidUpdate:(NSArray *)aConversationList {
    [self.channel invokeMethod:ExtSdkMethodKeyOnConversationUpdate
                     arguments:nil];
}

- (void)onConversationRead:(NSString *)from
                        to:(NSString *)to {
    [self.channel invokeMethod:ExtSdkMethodKeyOnConversationHasRead
...
- (void)messagesDidReceive:(NSArray *)aMessages {
    ...
    [self.channel invokeMethod:ExtSdkMethodKeyOnMessagesReceived
                     arguments:msgList];
}
...
- (void)cmdMessagesDidReceive:(NSArray *)aCmdMessages {
    ...
    [self.channel invokeMethod:ExtSdkMethodKeyOnCmdMessagesReceived
...
- (void)messagesDidRead:(NSArray *)aMessages {
    ...
    for (EMMessage *msg in aMessages) {
        ...
        [list addObject:json];
        [self.messageChannel invokeMethod:ExtSdkMethodKeyOnMessageReadAck
                                arguments:json];
    }
    
    [self.channel invokeMethod:ExtSdkMethodKeyOnMessagesRead arguments:list];
}
...
- (void)messagesDidDeliver:(NSArray *)aMessages {
    ...
    for (EMMessage *msg in aMessages) {
        NSDictionary *json = [msg toJson];
        [list addObject:json];
        [self.messageChannel invokeMethod:ExtSdkMethodKeyOnMessageDeliveryAck
                                arguments:@{@"message":json}];
    }
    
    [self.channel invokeMethod:ExtSdkMethodKeyOnMessagesDelivered
                     arguments:list];
}
...
- (void)messagesDidRecall:(NSArray *)aMessages {
    ...
    [self.channel invokeMethod:ExtSdkMethodKeyOnMessagesRecalled
                     arguments:list];
}

- (void)messageStatusDidChange:(EMMessage *)aMessage
                         error:(EMError *)aError {
    [self.messageChannel invokeMethod:ExtSdkMethodKeyOnMessageStatusChanged
...
- (void)groupMessageDidRead:(EMMessage *)aMessage groupAcks:(NSArray *)aGroupAcks {
    ...
    [self.channel invokeMethod:ExtSdkMethodKeyOnGroupMessageRead
                     arguments:list];
}
